...
::
@rem Query Registry for Cygwin install dir (Local Machine and Current Users - 64 and 32 bits)
...
IF NOT defined CYGWIN_PATH (
	...
	FOR /F "usebackq skip=2 tokens=1-2*" %%A IN (`REG QUERY %KEY_NAME% /v %VALUE_NAME% %CMPL_64% 2^>nul`) DO (
		SET CYGWIN_PATH=%%C
	)
)
IF NOT defined CYGWIN_PATH (
...
IF NOT defined CYGWIN_PATH (
	SET KEY_NAME=HKEY_LOCAL_MACHINE\SOFTWARE\Cygwin\setup
	FOR /F "usebackq skip=2 tokens=1-2*" %%A IN (`REG QUERY %KEY_NAME% /v %VALUE_NAME% %CMPL_32% 2^>nul`) DO (
		SET CYGWIN_PATH=%%C
	)
)
IF NOT defined CYGWIN_PATH (
...
@rem
echo Cygwin Path is %CYGWIN_PATH%
:Loop_Param
SET FILE_NAME=%1
IF NOT [%FILE_NAME%]==[] (
